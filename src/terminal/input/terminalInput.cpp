...
TerminalInput::OutputType TerminalInput::_searchWithModifier(const KEY_EVENT_RECORD& keyEvent) noexcept
{
    if (const auto match = _searchKeyMapping(keyEvent, s_modifierKeyMapping))
    {
        ...
        if (!v.sequence.empty())
        {
            ...
            StringType out;
            out.append(v.sequence);
            out[out.size() - 2] = L'1' + (shift ? 1 : 0) + (alt ? 2 : 0) + (ctrl ? 4 : 0);
            return out;
        }
    }
    ...
    else if (const auto match2 = _searchKeyMapping(keyEvent, s_simpleModifiedKeyMapping))
    {
        ...
        return _SendInputSequence(match2->sequence);
    }
    else
    {
        // One last check:
        ...
        {
            return _SendInputSequence(CTRL_ALT_QUESTIONMARK_SEQUENCE);
        }
        else if (ctrl && wasQuestionMark)
        {
            return _SendInputSequence(CTRL_QUESTIONMARK_SEQUENCE);
        }
        else if ((ctrl && alt) && wasSlash)
        {
            return _SendInputSequence(CTRL_ALT_SLASH_SEQUENCE);
        }
        else if (ctrl && wasSlash)
        {
            return _SendInputSequence(CTRL_SLASH_SEQUENCE);
        }
    }
...
TerminalInput::OutputType TerminalInput::HandleKey(const INPUT_RECORD& event)
{
    ...
    if (keyEvent.wVirtualKeyCode == VK_BACK)
    {
        ...
        {
            return _SendInputSequence({ &seq, 1 });
        }
    }
    ...
    {
        return _SendEscapedInputSequence(keyEvent.uChar.UnicodeChar);
    }
...
TerminalInput::OutputType TerminalInput::HandleFocus(const bool focused) const noexcept
{
    ...
    return _SendInputSequence(focused ? L"\x1b[I" : L"\x1b[O");
}
...
TerminalInput::OutputType TerminalInput::_SendChar(const wchar_t ch) noexcept
{
    StringType str;
...
// Return Value:
// - None
TerminalInput::OutputType TerminalInput::_SendEscapedInputSequence(const wchar_t wch) noexcept
{
    StringType str;
    str.push_back(L'\x1b');
    str.push_back(wch);
...
TerminalInput::OutputType TerminalInput::_SendInputSequence(const std::wstring_view& sequence) noexcept
{
    StringType str;
    str.append(sequence);
    return str;
...
TerminalInput::OutputType TerminalInput::_GenerateWin32KeySequence(const KEY_EVENT_RECORD& key) noexcept
{
    ...
    StringType str;
    fmt::format_to(std::back_inserter(str), FMT_COMPILE(L"\x1b[{};{};{};{};{};{}_"), vk, sc, uc, kd, cs, rc);
    return str;
}
