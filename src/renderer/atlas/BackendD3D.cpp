...
{
    // TODO: We upload the color bitmap too often.
    //
    // colorBitmapGeneration changes whenever any foreground or background color changes,
    // but we technically only need to upload the foreground only if it's needed by the shader,
    // which is only the case if any glyph has the LigatureMarker. I decided against making
    // this optimization at the time, because AtlasEngine.cpp bumps the colorBitmapGeneration
    // during scrolling anyways, which would shadow any optimization done here anyways.
    //
    // Not uploading this halves (!) the GPU load for any given frame.
