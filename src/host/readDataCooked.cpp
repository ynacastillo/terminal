...
bool COOKED_READ_DATA::_handleChar(wchar_t wch, const DWORD modifiers)
{
    ...
    case UNICODE_BACKSPACE:
        ...
        {
            const auto pos = TextBuffer::GraphemePrev(_buffer, _bufferCursor);
...
try
{
    ...
    case PopupKind::CommandNumber:
        ...
        break;
    case PopupKind::CommandList:
        popup.commandList.height = std::min<til::CoordType>(contentSize.height, _history->GetNumberOfCommands());
...
void COOKED_READ_DATA::_popupDrawCommandList(Popup& popup) const
{
    ...
    auto& cl = popup.commandList;
    const auto num = _history->GetNumberOfCommands();
    const auto w = popup.contentRect.narrow_width<size_t>();

    {
        cl.selected = std::clamp(cl.selected, 0, num - 1);

        if (cl.top > cl.selected)
        {
            cl.top = cl.selected;
        }
        else
        {
            const auto bottom = cl.top + cl.height;
            if (bottom <= cl.selected)
            {
                cl.top = cl.selected - cl.height + 1;
            }
        }

        cl.top = std::clamp(cl.top, 0, num - cl.height);
    }
    ...
    for (til::CoordType off = 0; off < cl.height; ++off)
    {
        ...
        buffer.append(L": ");
        buffer.append(_history->GetNth(historyIndex));
        buffer.append(w, L' ');
